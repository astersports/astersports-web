...
vi.mock("./_core/studio/ops/repeatDetector", () => ({
  checkRepeatAdvanced: vi.fn(),
}));
...
const mockRepeat = checkRepeatAdvanced as unknown as ReturnType<typeof vi.fn>;
...
beforeEach(() => {
  ...
  mockRepeat.mockReturnValue({ isRepeat: true, confidence: 0.8, classification: "ALLOVER", axes: [] });
...
describe("generateScaledImage", () => {
  it("signs a /manus-storage url, runs scalePrintRepeat, returns a PNG of the op RGBA", async () => {
    ...
    expect(mockScale).toHaveBeenCalledWith({ image: { url: "https://signed/x.jpg" }, fabric: fabric(true), targetFraction: 0.5, confirmedRepeat: true });
  ...
  it("throws NON_REPEAT_SCALE_ERROR when checkRepeatAdvanced returns isRepeat:false (Flag 2 non-repeat guard)", async () => {
    mockProvider.mockReturnValue({ getFabricMask: vi.fn().mockResolvedValue(fabric(true)) });
    mockSign.mockResolvedValue("https://signed/x.jpg");
    mockRepeat.mockReturnValue({ isRepeat: false, confidence: 0.1, classification: "PLACEMENT", axes: [] });
  ...
  it("calls checkRepeatAdvanced with the decoded image buffer and fabric raster data", async () => {
    mockProvider.mockReturnValue({ getFabricMask: vi.fn().mockResolvedValue(fabric(true)) });
    mockSign.mockResolvedValue("https://signed/x.jpg");
    const rgba = Buffer.alloc(2 * 2 * 4, 120);
    mockScale.mockResolvedValue({ data: rgba, width: 2, height: 2, changed: true });
