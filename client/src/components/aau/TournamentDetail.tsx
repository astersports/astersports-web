...
function GameCard({ g, multiDay }: { g: TournamentGame; multiDay: boolean }) {
  ...
  const teamRow = (name: string, score: number | null, won: boolean, lost: boolean) => (
    <div className="flex items-center justify-between gap-2 py-[3px]">
      <span className="truncate font-[var(--font-display)] text-[13.5px]" style={{ color: won ? C.ink : lost ? C.mut : C.dim, fontWeight: 600 }}>{name}</span>
      <span className="shrink-0 font-[var(--font-mono)] text-[17px] font-bold" style={{ color: live ? C.live : won ? C.pos : lit ? C.mut : C.faint }}>{score ?? "—"}</span>
    </div>
  );
  ...
  return (
    <div className="mb-[9px] rounded-[14px] p-[11px_12px]" style={{ border: `1px solid ${live ? "rgba(52,224,164,.28)" : C.hair}`, background: live ? "linear-gradient(160deg,rgba(52,224,164,.05),#121a2e)" : "#121a2e" }}>
      ...
      <div className="mt-[7px] flex flex-wrap items-center gap-2 pt-[8px] font-[var(--font-mono)] text-[10.5px]" style={{ color: C.mut, borderTop: `1px solid ${C.hair}` }}>
...
function Zone({ label, count }: { label: string; count?: string }) {
  return (
    <div className="mx-1 mb-[9px] mt-4 flex items-center gap-2 font-[var(--font-mono)] text-[10px] uppercase tracking-[0.16em]" style={{ color: C.mut }}>
      {label}
