...
export default function DivisionStandings({ divisionId, divisionName }: Props) {
  ...
  if (error) {
    return <div className="rounded-[16px] border border-[rgba(0,0,0,0.06)] bg-[linear-gradient(180deg,#F9FAFB,#FFFFFF)] p-6 text-center text-[13.8px] text-[#4B5563]">Couldn't reach the standings. Try again in a moment.</div>;
  }
  if (loading && !bundle) {
  ...
  return (
    <div className="space-y-4">
      <div>
        <div className="font-[var(--font-display)] text-[19.8px] font-bold tracking-tight text-[#1A1D23]">{divisionName}</div>
        <div className="mt-1 flex flex-wrap gap-2 text-[12.6px]">
        ...
        <div className="flex flex-wrap gap-1.5">
          {standings.map((r) => (
            <button
              ...
              className={`rounded-full border px-3 py-1.5 text-[12.6px] font-semibold transition-colors ${
