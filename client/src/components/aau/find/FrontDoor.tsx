...
// stays mounted across modes). Live strip (reads on load, hides if empty) + a recency-ordered
// slice of the real directory (no fabricated rows, no "this weekend" framing it can't back).

export default function FrontDoor({ dir, onOpen }: { dir: DirTournament[] | null; onOpen: (t: DirTournament) => void }) {
  const recent = (dir ?? []).slice(0, 5);
  return (
