  ...
  return (
    ...
    >
      ...
      <span className="min-w-0 flex-1">
        <span className="flex items-center gap-[7px] text-[15px] font-medium" style={{ color: C.ink }}>
          <span className="truncate">{v.name}</span>
          {v.isLive && (
            <span className="flex shrink-0 items-center gap-1 text-[11.5px]" style={{ color: C.live }}>
              <span
        ...
        <span className="mt-[3px] flex flex-wrap items-center gap-[5px]">
          ...
          <span className="font-[var(--font-mono)] text-[11.5px]" style={{ color: hasRecord ? C.pos : C.mut }}>
            {hasRecord ? `${v.record.w}–${v.record.l}` : "0–0"}
          </span>
          <span className="font-[var(--font-mono)] text-[11.5px]" style={{ color: v.basis ? C.g3 : C.mut }} title="rating · a projection, distinct from the W–L record">
            rtg {ratingLabel}
          </span>
        </span>
      </span>
