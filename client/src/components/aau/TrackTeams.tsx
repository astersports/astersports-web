  ...
  return (
    <div className={`as-fade-in ${bottomPad}`}>
      <button
        type="button"
        onClick={onBack}
        className="mb-2 flex items-center gap-1 font-[var(--font-mono)] text-[12.6px] text-[#374151]"
      >
        <ChevronLeft className="h-4 w-4" /> Find
      </button>

      <div className="px-[18px]">
        <div className="font-[var(--font-mono)] text-[11.5px] uppercase tracking-[0.12em] text-[#8F6708]">
          {tournamentName}
        </div>
        <h2 className="mt-1 font-[var(--font-display)] text-[26.5px] font-bold tracking-[-0.3px] text-[#1A1D23]">
          Track teams
        </h2>
      </div>
      ...
      <div className="mx-[18px] mt-2 flex items-center gap-[11px] rounded-[15px] border border-[#E2E8F0] bg-[#F9FAFB] px-[15px] py-[13px] shadow-[inset_0_1px_0_rgba(0,0,0,0.04)]">
        ...
        <input
          ...
          className="w-full bg-transparent text-[14.1px] text-[#1A1D23] placeholder-[#4B5563] outline-none"
        />
      </div>
          ...
          {availGrades.length > 0 && (
            <div className="px-[18px]">
              <button
                ...
                className="as-press inline-flex min-h-[32px] items-center gap-[6px] rounded-full px-[12px] font-[var(--font-mono)] text-[12.6px]"
                style={
      ...
      {/* states */}
      {error && (
        <div className="mx-[18px] mt-4 rounded-[15px] border border-[rgba(0,0,0,0.06)] bg-[linear-gradient(180deg,#F9FAFB,#FFFFFF)] p-6 text-center text-[13.8px] text-[#4B5563]">
      ...
      {data && filtered.length === 0 && (
        <div className="mx-[18px] mt-4 rounded-[15px] border border-[rgba(0,0,0,0.06)] bg-[linear-gradient(180deg,#F9FAFB,#FFFFFF)] p-8 text-center">
          ...
          <div className="text-[14.6px] font-semibold text-[#1A1D23]">No teams match</div>
          <div className="mt-1 text-[13.8px] text-[#4B5563]">Clear the filters or try a different search.</div>
        </div>
      )}
            ...
            {nested && (
                  ...
                  >
                    <span>
                      ...
                      <small className="mt-0.5 block font-[var(--font-mono)] text-[11.5px] text-[#4B5563]">
                        {teams.length} teams in this view
                      </small>
                    </span>
                    <span className="font-[var(--font-mono)] text-[12.6px] text-[#8F6708]">{allOn ? "Clear" : "+ All"}</span>
                  </button>
            )}
            {teams.map((t) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    <span className="block truncate text-[14.1px] font-semibold text-[#1A1D23]">{t.name}</span>
                    <small className="mt-0.5 block font-[var(--font-mono)] text-[11.5px] text-[#4B5563]">
      ...
      {sel.size > 0 && (
        <div className="fixed inset-x-0 bottom-[calc(64px+env(safe-area-inset-bottom))] z-30 bg-[linear-gradient(transparent,#F7F8FA_40%)] px-[18px] pb-3 pt-5">
          <button
            type="button"
            onClick={doTrack}
            className="as-press flex w-full items-center justify-center gap-2 rounded-[14px] bg-[linear-gradient(100deg,#E0631C,#E8902A,#F6CC55,#FBD56B)] py-[15px] font-[var(--font-display)] text-[16.7px] font-semibold text-[#1a1206] shadow-[0_12px_30px_-12px_rgba(224,99,28,0.6)]"
          >
      ...
      {items.map((it) => {
        ...
        return (
          <button
            ...
            type="button"
            onClick={() => onPick(it)}
            className={`shrink-0 rounded-full border px-[15px] py-[7px] text-[14.4px] font-semibold ${
              on
