...
export default function HubAccount({ user }: { user: HubUser | null }) {
  ...
  if (!user) {
    return (
      <button
        ...
        className="as-press inline-flex min-h-[44px] items-center gap-1.5 rounded-full border border-[#E2E8F0] bg-[#FFFFFF] px-3 py-[6px] text-[12.6px] font-semibold text-[#374151] disabled:opacity-60"
      >
  ...
  return (
    <div className="relative">
      ...
      >
        <span className="grid h-7 w-7 place-items-center overflow-hidden rounded-full bg-[linear-gradient(135deg,#E8902A,#F6CC55)] text-[13.8px] font-bold text-[#1a1206]">
        ...
        <>
          ...
          <div role="menu" aria-label="Account" className="absolute right-0 top-[46px] z-50 w-48 overflow-hidden rounded-[12px] border border-[#E2E8F0] bg-[#FFFFFF] p-1 shadow-[0_12px_30px_-12px_rgba(0,0,0,0.18)]">
            <div className="px-3 py-2">
              <div className="truncate text-[13.8px] font-semibold text-[#1A1D23]">{user.name ?? "Signed in"}</div>
              {user.email && <div className="truncate text-[11.5px] text-[#4B5563]">{user.email}</div>}
            </div>
            <button
              type="button"
              role="menuitem"
              onClick={() => { setOpen(false); void signOutHub(); }}
              className="as-press min-h-[44px] w-full rounded-[8px] px-3 py-2 text-left text-[13.8px] text-[#DC2626] hover:bg-[rgba(255,107,94,0.08)]"
            >
              Sign out
            </button>
          </div>
